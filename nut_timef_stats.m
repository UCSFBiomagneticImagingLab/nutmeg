...
% Last Modified by GUIDE v2.5 03-Aug-2015 22:20:56
...
if stats.markmode>0
    ...
    if stats.markmode==1
        ...
        if stats.docomp, errordlg('Population files cannot be used (yet?) for comparisons between conditions.'), return, end
    ...
    case 'anovabet_snpm'
        ...
        if strcmp(get(handles.nut_edit_condnr,'string'),'all')
            ...
            if size(stats.condnr,1)>1, errordlg('No semicolons allowed in condition field for ANOVA.'), return, end
            if ~isequal(length(stats.condnr),length(stats.subjnr))
                errordlg('You must specify a condition for each subject'), return
            end            
        end
        stats.comps=unique(stats.condnr);
        stats.condsel=stats.comps;        
    ...
    stats.subjnr  = stats.subjsel;
    switch stats.type
    case {'ttest2_snpm' 'anovabet_snpm'}
        stats.condnr = str2num(get(handles.nut_edit_condnr,'string'));   
        if isempty(stats.condnr)
...
if stats.dostat
    ...
    switch stats.type  
        ...
        case 'anovabet_snpm'
            tmp = zeros(1,length(stats.condsel));
            for i = 1:length(stats.comps)
               tmp(i) = sum(stats.condnr==i);
            end
            autonumperm = round(exp(gammaln(stats.numsubj+1)-sum(gammaln(tmp+1))));
...
switch options{get(hObject,'value')}
    ...
    case 'SnPM within-subject ANOVA'
        ...
        stats.doavg=false;
        stats.docomp=false;    
    case 'SnPM between-subject ANOVA'
        set([handles.text_covar handles.nut_edit_corrvar],'Enable','off')        
        set([handles.text41 handles.nut_edit_numperm],'Enable','on')
        set([handles.nut_radio_tmeanstat handles.text48 handles.nut_radio_tone handles.nut_radio_tsum],'Enable','on')
        if strcmp(stats.tsel,'one'), set(handles.nut_check_corr4multtime,'Enable','on'), end
