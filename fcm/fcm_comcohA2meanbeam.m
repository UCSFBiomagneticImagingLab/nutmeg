...
switch lower(typ)
    ...
    case 'interhomo'
        if useroi
            ...
            for vv=1:nv
                ...
                %distances = nut_rownorm(nut_coord_diff(MRIvoxels(e,:),cc));
                f = find( dist(vv,e) < 1.5*max(nuts.voxelsize) );
    ...
    case 'interhetero'
        if useroi
            ...
            for vv=1:nv
                e = find( voxsign~=sign(MRIvoxels(vv,1)) );
                f = ( dist(vv,e)>2*max(nuts.voxelsize) );
                temp(vv,:,:)=mean(CC.coh(CM(vv,e(f)),:,:),1);
            end
        end
    case 'intra'      
